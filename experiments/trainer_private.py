...
class TrainerPrivate(object):
    ...
    def train(self, e, dataloader, attacker, wm_dataloader=None):
        self.model.train()
        loss_meter = 0
        sign_loss_meter = 0
        public_acc_meter = 0
        ...
        for i, (data, target) in enumerate(dataloader):
            ...
            noise = torch.randn_like(data, device='cuda') * 0.01
            data = attacker.attack(self.model, data, target,
                                        noise=noise, num_noise_vectors=1, no_grad=True)
            data = data + noise
    ...
    def test(self, dataloader, attacker=None, msg='Testing Result'):
        self.model.eval()
            ...
            with torch.no_grad():
                for j, load in enumerate(dataloader):
                    ...
                    if attacker is not None:
                        data = attacker.attack(self.model, data, target, noise)
